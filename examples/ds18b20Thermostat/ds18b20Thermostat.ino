/*
  ...
  Example - DS18B20 Thermostat
  The following sketch creates a thermostat with a DS18B20 temperature sensor, sets desired temperature and threesolds and handles it, shows temperature and humidity values got from the sensor.
*/
#include <SimpleThermostat.h>
SimpleThermostat thermostat(2, 3, 4, TYPE_DS18B20); //create a thermostat with cooling device on pin 2, heating device on pin 3 and a DS18B20 sensor on pin 4
void setup() {
  Serial.begin(9600);
  Serial.println("SimpleThermostat library");
  Serial.println("Created by EPMatt");
  Serial.println("DS18B20 Thermostat Example");
  ...
  //set lower threesold: the heating pin will be turned on when temperature reaches 22.5-1.5 = 21.0 °C
...
void loop() {
  ...
  if (temperature == ERROR_UNABLE_TO_READ_SENSOR || humidity == ERROR_UNABLE_TO_READ_SENSOR) {
    ...
    //when trying to get humidity value on a DS18B20 sensor, getCurrentHum() returns ERROR_UNSUPPORTED_OPERATION
  }
