/*
  ...
  Example - DHT11 Thermostat
  The following sketch creates a thermostat with a DHT11 temperature sensor, sets desired temperature and threesolds and handles it, shows temperature and humidity values got from the sensor.
*/
#include <SimpleThermostat.h>
SimpleThermostat thermostat(2, 3, 4, TYPE_DHT11); //create a thermostat with cooling device on pin 2, heating device on pin 3 and a DHT11 sensor on pin 4
void setup() {
  Serial.begin(9600);
  Serial.println("SimpleThermostat library");
  Serial.println("Created by EPMatt");
  Serial.println("DHT11 Thermostat Example");
